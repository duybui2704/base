...
function formatDateSecondPicker(date: number) {
  return Moment(date).utc(true).format(DEFAULT_DATE_FORMAT);
}
...
function getCurrentDateTime() {
  return Moment(Moment().valueOf()).utc(true).format(FULL_DATE_FORMAT);
}
...
function formatDateToTicks(date: string) {
  return (
    Moment(date, DEFAULT_DATE_FORMAT).utc(true).hours(23).minutes(59).seconds(59).valueOf() *
      10000 +
    621355968000000000
  );
}

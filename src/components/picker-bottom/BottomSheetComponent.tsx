...
const BottomSheetComponent = forwardRef<BottomSheetAction, BottomSheetProps>(
  (
    ...
    const searchItem = useCallback(
      (text: string) => {
        if (text) {
          if (!isValueBank) {
            setDataFilter(
              data?.filter(item => item?.value?.toUpperCase().includes(text.toUpperCase())),
            );
          } else {
            setDataFilter(
              data?.filter(item =>
    ...
    const renderItem = useCallback(
          ...
          <>
            <Touchable onPress={onPress} style={styles.valueContainer}>
              <View style={styles.row}>
                ...
                <Text style={!leftIcon ? styles.value : styles.noLeftIconValue}>
                  {isValueBank ? `${item.value}${' - '}${item.text}` : item.value}
                </Text>
                {rightIcon}
              </View>
            </Touchable>
      ...
      [hide, isValueBank, leftIcon, onPressItem, rightIcon],
    );
      ...
      <BottomSheetModal
        ...
        {hasInputSearch && (data?.length as number) >= MIN_SIZE_HAS_INPUT && (
          <BottomSheetTextInput
            ...
            value={textSearch}
            style={[styles.input, {borderColor: focus ? COLORS.BLUE : COLORS.GRAY}]}
